...
export const pathGuard: BuildingBlock = {
  ...
  explain: {
    allowOnce: "write to a source path instead; build output and dependency directories are regenerated by their tools",
    change: "harness.yaml > hooks > path-guard > params.blockedPaths: remove the path (protectHarness: false lifts the self-protection), then omh sync",
  },
  template: `#!/bin/bash
set -euo pipefail
INPUT=$(cat)
