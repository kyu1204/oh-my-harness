...
describe("doctorCommand", () => {
  ...
  it("hints migration when config still uses the deprecated codex_hooks flag", async () => {
    await setupInitializedProject(tmpDir);
    await fs.writeFile(
      path.join(tmpDir, ".codex", "config.toml"),
      "[features]\ncodex_hooks = true\ngoals = true\n",
    ...
    expect(result.checks.codexConfig).toBe(false);
    expect(result.healthy).toBe(false);
    expect(result.messages.some((m) => m.includes("deprecated") && m.includes("codex_hooks"))).toBe(true);
  });

  it("accepts a valid Pi bridge extension when present", async () => {
    await setupInitializedProject(tmpDir);
    await fs.mkdir(path.join(tmpDir, ".pi", "extensions"), { recursive: true });
    await fs.writeFile(
      path.join(tmpDir, ".pi", "extensions", "omh-harness.ts"),
      '// AUTO-GENERATED by oh-my-harness. Do not edit by hand.\nexport default function (pi) { pi.on("tool_call", async () => undefined); }\n',
      "utf-8",
    );

    const result = await doctorCommand({ projectDir: tmpDir });

    expect(result.checks.piConfig).toBe(true);
    expect(result.healthy).toBe(true);
  });

  it("treats an absent Pi extension as acceptable (only generated when PreToolUse hooks exist)", async () => {
    await setupInitializedProject(tmpDir);
    const result = await doctorCommand({ projectDir: tmpDir });
    expect(result.checks.piConfig).toBe(true);
  });

  it("reports unhealthy when the Pi extension exists but is corrupt", async () => {
    await setupInitializedProject(tmpDir);
    await fs.mkdir(path.join(tmpDir, ".pi", "extensions"), { recursive: true });
    await fs.writeFile(
      path.join(tmpDir, ".pi", "extensions", "omh-harness.ts"),
      "<<< not a bridge extension",
      "utf-8",
    );

    const result = await doctorCommand({ projectDir: tmpDir });

    expect(result.checks.piConfig).toBe(false);
    expect(result.healthy).toBe(false);
    expect(result.messages.some((m) => m.includes(".pi/extensions/omh-harness.ts"))).toBe(true);
  });
