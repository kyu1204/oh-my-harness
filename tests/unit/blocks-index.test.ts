...
describe("blocks index", () => {
  it("exports all 25 catalog blocks", () => {
    ...
    expect(ids).toContain("stop-test-gate");
    expect(ids).toContain("stop-uncommitted-warn");
    expect(ids).toContain("semantic-rule-guard");
    expect(ids).toContain("semantic-diff-gate");
    expect(builtinBlocks.length).toBe(25);
  });
});
